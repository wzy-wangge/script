...
docker run --name=wxedge2 --restart=always --privileged --net=host --tmpfs /run --tmpfs /tmp -e REC=false -e LISTEN_ADDR=":7998" -v /data2/wxedge_storage:/storage:rw --log-opt max-size=50m -d registry.cn-chengdu.aliyuncs.com/wzy_111/wxedge:2.4.1
if [[ $? -eq 0 ]]; then
   echo  "二开网心云容器成功"
fi
